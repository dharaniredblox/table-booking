...
async function scrapeRestaurant(page, restaurantUrl) {
  // console.log(`🍽 Scraping: ${restaurantUrl}`);
  ...
  // const nameRaw = rs.name || $("h1, h2").first().text().trim() || "unknown";

  // ✅ Clean restaurant name before generating ID
  ...
  const cleaned = {
    ...
    Venue_Information: {
      ...
      cuisineType: Array.isArray(rs.servesCuisine)
        ? rs.servesCuisine.join(", ")
        : rs.servesCuisine || "",
      address: address || "",
      postcode,
      latitude,
...
async function main() {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      const ok = await uploadCleanedJsonToS3(cleaned.restaurantId, cleaned);
      if (ok) {
        // update booking table status (restaurant_id used as key)
        try {
          await updateScrapeStatus(cleaned.restaurantId, "completed");
        } catch (err) {
          console.warn(
            `⚠️ Could not update scrape status for ${cleaned.restaurantId}:`,
            err.message
          );
        }
      }
