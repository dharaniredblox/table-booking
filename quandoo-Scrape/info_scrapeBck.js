...
async function uploadCleanedJSONToS3(cleanedJSON, filename) {
  await s3
    ...
    .promise();
  console.log(`☁️ Uploaded cleaned JSON to S3: ${filename}`);
}
...
async function scrapeRestaurantDetails(restaurant, page) {
  ...
  try {
    ...
    console.log("grabPillsBelow",grabPillsBelow);
    ...
    console.log(`✅ Updated DynamoDB status for ${resId}`);
    ...
    console.log(`✅ Scraped & cleaned: ${resId}`);
